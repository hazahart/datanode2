...
echo ">>> 2. Instalando dependencias (Java 17, SSH, etc)..."
apt-get update
# Se actualiza el paquete a openjdk-17-jdk para compatibilidad con Debian Trixie/Sid
apt-get install -y openjdk-17-jdk ssh pdsh wget tar curl

echo ">>> 3. Creando usuario '$HADOOP_USER'..."
if ! id "$HADOOP_USER" &>/dev/null; then
    useradd -r -m -d /home/$HADOOP_USER -s /bin/bash $HADOOP_USER
fi
# Aseguramos permisos en el home del usuario
...
# Limpiamos instalaciones previas rotas para evitar errores de movimiento
rm -rf $INSTALL_DIR
cd /tmp
# Forzamos IPv4 con -4 para evitar el error de red de WSL
...
# Limpiamos entradas duplicadas si existen
...
# Actualizamos el hadoop-env.sh con la ruta de Java 17
...
echo "Recuerda ejecutar: sudo service ssh start"
echo "======================================================"
